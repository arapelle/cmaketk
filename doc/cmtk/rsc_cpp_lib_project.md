...
# Resource Cpp Library Project

## Include
`find_package(cmaketk COMPONENTS rsc_cpp_lib_project)`

## Functions
### Function `cmtk_add_rsc_cpp_library(target_name SHARED|STATIC ...)`

&ensp;&ensp;&ensp;&ensp;Create a shared or static C++ library target embedding serialized resource files.

&ensp;&ensp;&ensp;&ensp;The function takes a list of resource files and copy their contents in a C++ library.
...
Indeed, a function `find_serialized_resource(resource_path)` returns an `optional<span<const byte>>` corresponding
...
&ensp;&ensp;&ensp;&ensp;To generate the library, the function takes some arguments. The *NAMESPACE* determines
two important aspects: the name of the main directory and the main C++ namespace of the generated code.
All generated files are in this main directory (or a one of its subdirectories), and every constants and functions are declared
in this main C++ namespace. (It is possible to provide a context namespace which contains the main namespace.
A context directory containing the main directory is created too in this case.)
...
&ensp;&ensp;&ensp;&ensp;The tool headers are directly under the main directory. The context directory is never present
...
(or *context-namespace/main-directory*/ if you provide a parent namespace).
...
- NAMESPACE *namespace* :  A namespace defining the main directory containing the generated code files, 
and also the namespace containing all generated constants, types and functions.
- [CONTEXT_NAMESPACE *namespace*] :  A namespace containing the main namespace defined by *NAMESPACE*. It generates a directory parent to the main directory too.
- [INLINE_CONTEXT_NAMESPACE *namespace*] :  An inline namespace containing the main namespace defined by *NAMESPACE*. The parent directory is created too.
...
&ensp;&ensp;&ensp;&ensp;The following arguments of [`cmtk_add_cpp_library(target_name SHARED|STATIC ...)`](CppLibraryProject.md) are compatible with this function:
- CXX_STANDARD
- HEADERS
- SOURCES
- HEADERS_BASE_DIRS
...
`-- rsc/
    ...
    `-- plants/
        ...
        `-- daisy.png
```
The function `cmtk_add_rsc_cpp_library()` is called with the following parameters:
```cmake
cmtk_add_rsc_cpp_library(myrsclib SHARED
    NAMESPACE myrsclib
    RESOURCES
