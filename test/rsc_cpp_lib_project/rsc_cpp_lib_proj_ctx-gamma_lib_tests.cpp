...
TEST(rsc_cpp_lib_proj_ctx_gamma_lib_tests, configure__ok)
{
...
TEST(rsc_cpp_lib_proj_ctx_gamma_lib_tests, build__ok)
{
    const int res = cmake.build(BUILD_DIR);
    ASSERT_EQ(res, 0);
}

TEST(rsc_cpp_lib_proj_ctx_gamma_lib_tests, test__ok)
{
...
TEST(rsc_cpp_lib_proj_ctx_gamma_lib_tests, install__ok)
{
    const int res = cmake.install(BUILD_DIR);
    ASSERT_EQ(res, 0);
}

TEST(rsc_cpp_lib_proj_ctx_gamma_lib_tests, test_package__ok)
{
...
TEST(rsc_cpp_lib_proj_ctx_gamma_lib_tests, uninstall__ok)
{
